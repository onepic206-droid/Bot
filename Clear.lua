...
function finalSweep()
    ...
    for _, tile in pairs(GetTiles()) do
        if tile.fg == 3 and tile.extra and tile.extra.progress >= 1 then
            storeItems()
            FindPath(tile.x, tile.y, 400)
            moveAct("hit", tile.x, tile.y)
            collect(3)
        end
    end
...
function pPlatform()
    ...
    if myPlatCount > 0 and GetItemCount(Config.PLAT_ID) < myPlatCount then
        ...
        while GetItemCount(Config.PLAT_ID) < myPlatCount do
            ...
            FindPath(math.floor(item.pos.x / 32), math.floor(item.pos.y / 32), 400)
            collect(2)
            Sleep(500)
        end
    ...
    for _, x in pairs({1, 98}) do
        for py = 2, 52, 2 do
            while GetTile(x, py).fg == 0 do
                FindPath(x, py - 1, 400)
                moveAct("place", x, py, Config.PLAT_ID)
            end
        end
    end
end
    ...
    local function farmSeeds()
        while GetItemCount(2) == 0 do
            ...
            for x = 2, 22 do
                ...
                if tile.fg == 3 and tile.extra and tile.extra.progress >= 1 then
                    FindPath(x, plantY, 400)
                    moveAct("hit", x, plantY)
                    collect(2)
                elseif tile.fg == 0 and GetItemCount(3) > 0 then
                    FindPath(x, plantY, 400)
                    moveAct("place", x, plantY, 3)
                end
            end
            Sleep(500)
        end
...
sendWebhook("Semua World Selesai!")
